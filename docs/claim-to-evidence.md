...
| FHIR R4 alignment. | `services/fhir-adapter/mapper.ts`, `services/fhir-adapter/vcemMapper.ts` | Fixture mapper to VCEM lifecycle payloads and AuditEvent-shaped output. | `test/FHIRAdapter.ts`, `fixtures/fhir/*`, `docs/fhir-mapping.md` | No live FHIR server integration or production profile validation. |
